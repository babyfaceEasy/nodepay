      ...
      <div class="col-md-7 col-xs-12 mb20 marginTopPlus">
        <div class="card">
          ...
          <div class="wap-wed mt20 mb20">
            ...
            <p style="text-align: center; line-height:25px;" class="mb20"><strong>{{$fromWallet->currency->code}}</strong> of <strong>{{ formatNumber($transInfo['defaultAmnt']) }}</strong>
              @lang('message.dashboard.exchange.confirm.has-exchanged-to') <strong>
                {{  moneyFormat($transInfo['currCode'], isset($transInfo['finalAmount']) ? formatNumber($transInfo['finalAmount']) : 0.00) }}

              </strong><br/>@lang('message.dashboard.exchange.confirm.exchange-rate'):<strong> 1 {{$fromWallet->currency->code}}</strong> = {{ formatNumber($transInfo['dCurrencyRate']) }}
