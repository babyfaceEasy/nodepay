							                     ...
							                     <div class="wap-wed mt20 mb20">
													   <hr>
														...
														<div class="form-group">
																...
																<input type="email" class="form-control email" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter email" value="{{$requestPayment->email}}" name="email" id="email" readonly>
...
function checkAmountLimitAndFeesLimit(){
	    	...
	    	$.ajax({
				...
				dataType: "json",
				data: { "_token":token,'amount':amount,
			...
			.done(function(response) {
				...
				// if(response.success.status == 101)
				// {
				...
				if(response.success.status == 200)
				{
					...
					if(response.success.totalAmount > response.success.balance || response.success.balance == 0)
					{
                        $('#rp_money').attr('disabled','true');
						if (response.success.checkingRequestedCurrency == 404)
						{
