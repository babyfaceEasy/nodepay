                ...
                <table style="width:100%; border-radius:1px;  border-collapse: collapse;">
                    ...
                    <tr style="background-color:#fff; text-align:center; font-size:12px; font-weight:normal;">
                        ...
                        <td>
                          @if($exchange->type == 'Out')
                            @if ($exchange->amount > 0)
                                {{  moneyFormat($def_curr_symbol, formatNumber($exchange->amount)) }}
                            @endif
                          @elseif($exchange->type == 'In')
                            @if ($exchange->amount > 0)
                                {{  moneyFormat($exchange->tc_symbol, formatNumber($exchange->amount)) }}
                            @endif
                          @endif
                        </td>
                        ...
                        <td>
                          @if($exchange->type == 'Out')
                            @if ($total > 0)
                                {{ moneyFormat($def_curr_symbol, formatNumber($total)) }}
                            @endif
                          @elseif($exchange->type == 'In')
                            @if ($total > 0)
                                {{ moneyFormat($exchange->tc_symbol, formatNumber($total)) }}
                            @endif
                          @endif
                        </td>
