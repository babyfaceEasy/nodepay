                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-6">
                      <select name="date_format" class="select2" >
                          <option value="0" {{isset($prefData['preference']['date_format']) && $prefData['preference']['date_format'] == 0 ? 'selected':""}}>yyyymmdd {2018 12 31}</option>
                          <option value="1" {{isset($prefData['preference']['date_format']) && $prefData['preference']['date_format'] == 1 ? 'selected':""}}>ddmmyyyy {31 12 2018}</option>
                          <option value="2" {{isset($prefData['preference']['date_format']) && $prefData['preference']['date_format'] == 2 ? 'selected':""}}>mmddyyyy {12 31 2018}</option>
                          <option value="3" {{isset($prefData['preference']['date_format']) && $prefData['preference']['date_format'] == 3 ? 'selected':""}}>ddMyyyy &nbsp;&nbsp;&nbsp;{31 Dec 2018}</option>
                          <option value="4" {{isset($prefData['preference']['date_format']) && $prefData['preference']['date_format'] == 4 ? 'selected':""}}>yyyyMdd &nbsp;&nbsp;&nbsp;{2018 Dec 31}</option>
                      </select>
                    </div>
                  </div>
    ...
    <script type="text/javascript">
        ...
        //two_step_verification
          $(document).ready(function()
          {
            ...
            {
                // alert('here');
                if ($(this).val() == 'by_phone' || $(this).val() == 'by_email_phone')
                {
                  $.ajax({
                    headers:
                    {
          ...
          $("#phone_verification").change(function()
          {
              ...
              if ($(this).val() == 'Enabled')
              {
                ...
                .done(function(response)
                {
                    ...
                    if (response.status == false)
                    {
                        $('#phone_verification-error').addClass('error').html(response.message).css("font-weight", "bold");
                        $('form').find("button[type='submit']").prop('disabled',true);
                    }
                    else if (response.status == true)
                    {
                        $('#phone_verification-error').html('');
