...
@section('page_content')
    <div class="box">
        <div class="box-body">
            <form class="form-horizontal" action="{{ url('admin/transfers') }}" method="GET">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-control select2" name="currency" id="currency">
                                    ...
                                    @foreach($transfer_currencies as $transfer)
                                        <option value="{{ $transfer->currency_id }}" {{ ($transfer->currency_id == $currency) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-2">
                                ...
                                <select class="form-control select2" name="status" id="status">
                                    ...
                                    @foreach($transfer_status as $transfer)
                                      <option value="{{ $transfer->status }}" {{ ($transfer->status == $status) ? 'selected' : '' }}>
                                        {{
                                            (
                                                ($transfer->status == 'Blocked') ? "Cancelled" :
                                                (
                                                    ($transfer->status == 'Refund') ? "Refunded" : $transfer->status
                                                )
                                            )
                                        }}
                                      </option>
...
<script type="text/javascript">
    ...
    $(document).ready(function()
    {
        ...
        $('#user_input').autocomplete(
        {
            source:function(req,res)
            {
                ...
                {
                    $.ajax({
                        url:'{{url('admin/transfers/user_search')}}',
                        dataType:'json',
                        type:'get',
                        data:{
                            search:req.term
                        ...
                        success:function (response)
                        {
                            ...
                            if(response.status == 'success')
                            {
                                ...
                                {
                                    return {
                                            sender_id : item.sender_id, //sender_id is defined
    ...
    $(document).ready(function()
    {
        $('#csv').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/admin/transfers/csv?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
          +"&currency="+currency
          +"&user_id="+user_id;
    ...
    $(document).ready(function()
    {
        $('#pdf').on('click', function(event)
        {
          ...
          window.location = SITE_URL+"/admin/transfers/pdf?startfrom="+startfrom
          +"&endto="+endto
          +"&status="+status
          +"&currency="+currency
          +"&user_id="+user_id;
