...
{
    $wallet            = App\Models\Wallet::where(['user_id' => \Auth::user()->id, 'is_default' => 'Yes'])->first();
...
{
    $data = Pages::where('position', 'like', "%$position%")->where('status', 1)->get();
    return $data;
}

function getSocialLink()
{
    $data = collect(DB::table('socials')->get())->toArray();
    return $data;
}
...
{
    $app = App\Models\AppStoreCredentials::where(['has_app_credentials' => 'Yes'])->get();
    return $app;
}
...
function getfavicon()
{
    ...
    if (!$session)
    {
        $session = \App\Models\Setting::where(['name' => 'favicon', 'type' => 'general'])->first();
...
function getCompanyLogo()
{
    ...
    if (!$session)
    {
        $session = \App\Models\Setting::where(['name' => 'logo', 'type' => 'general'])->first();
        $session = $session->value;
...
function coinPaymentInfo()
{
    ...
    $cpm       = CurrencyPaymentMethod::where(['method_id' => $transInfo['payment_method'], 'currency_id' => $transInfo['currency_id']])->first();
