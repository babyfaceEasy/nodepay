...
class TransactionController extends Controller
{
    ...
    public function index(TransactionsDataTable $dataTable)
    {
        ...
        $data['t_currency'] = $t_currency = Transaction::select('currency_id')->groupBy('currency_id')->get();

        $data['t_type'] = $t_type = Transaction::select('transaction_type_id')->whereNotIn('transaction_type_id', [Voucher_Created, Voucher_Activated])->groupBy('transaction_type_id')->get();
    ...
    public function transactionCsv()
    {
        ...
        if (!empty($transaction))
        {
            foreach ($transaction as $key => $value)
            {
                ...
                elseif ($value->transaction_type_id == Transferred)
                {
                    $datas[$key]['Receiver'] = isset($value->transfer->receiver) ? $value->transfer->receiver->first_name . ' ' . $value->transfer->receiver->last_name : $value->transfer->email;
                }
                elseif ($value->transaction_type_id == Bank_Transfer)
                {
                    $datas[$key]['Receiver'] = "-";
    ...
    public function update(Request $request, $id)
    {
        ...
        $requestToTypeTransactionEntry = Transaction::where(['transaction_type_id' => Request_To, 'uuid' => $request->uuid, 'transaction_reference_id' => $request->transaction_reference_id])->select('percentage', 'charge_percentage', 'charge_fixed')->first();
        ...
        //Withdrawal
        $englishTempInfoWithdrawal = EmailTemplate::where(['temp_id' => 10, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first(); //if other language's not set, get en sub and body

        $w_success_temp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type'        => 'email',
        ])->select('subject', 'body')->first();

        $w_penfing_temp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type'        => 'email',
        ])->select('subject', 'body')->first();

        $w_cancel_tmp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type'        => 'email',
        ])->select('subject', 'body')->first();
        ...
        if ($request->type == 'Withdrawal')
        {
            if ($request->status == 'Success') //requested status
            {
                ...
                elseif ($t->status == 'Blocked') //current status
                {
                    ...
                    // Withdrawal Mail
                    if (!empty($w_success_temp->subject) && !empty($w_success_temp->body))
                    {
                        $w_success_sub = str_replace('{uuid}', $withdrawal->uuid, $w_success_temp->subject);
                        $w_success_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $w_success_temp->body);
                    }
                    else
                    {
                        $w_success_sub = str_replace('{uuid}', $withdrawal->uuid, $englishTempInfoWithdrawal->subject);
                        $w_success_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishTempInfoWithdrawal->body);
                    }
            ...
            elseif ($request->status == 'Pending') //requested status
            {
                ...
                elseif ($t->status == 'Success') //current status
                {
                    ...
                    // Withdrawal Mail
                    if (!empty($w_penfing_temp->subject) && !empty($w_penfing_temp->body))
                    {
                        $w_pending_sub = str_replace('{uuid}', $withdrawal->uuid, $w_penfing_temp->subject);
                        $w_pending_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $w_penfing_temp->body);
                    }
                    else
                    {
                        $w_pending_sub = str_replace('{uuid}', $withdrawal->uuid, $englishTempInfoWithdrawal->subject);
                        $w_pending_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishTempInfoWithdrawal->body);
                    }
            ...
            elseif ($request->status == 'Blocked') //requested status
            {
                ...
                elseif ($t->status == 'Pending') //current status
                {
                    ...
                    // Withdrawal Mail
                    if (!empty($w_cancel_tmp->subject) && !empty($w_cancel_tmp->body))
                    {
                        $w_cancel_sub = str_replace('{uuid}', $withdrawal->uuid, $w_cancel_tmp->subject);
                        $w_cancel_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $w_cancel_tmp->body);
                    }
                    else
                    {
                        $w_cancel_sub = str_replace('{uuid}', $withdrawal->uuid, $englishTempInfoWithdrawal->subject);
                        $w_cancel_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishTempInfoWithdrawal->body);
                    }
                ...
                elseif ($t->status == 'Success') //current status
                {
                    ...
                    $w_cancel_msg = str_replace('{uuid}', $withdrawal->uuid, $w_cancel_msg);
                    $w_cancel_msg = str_replace('{status}', ($withdrawal->status == 'Blocked') ? 'Cancelled' : $withdrawal->status, $w_cancel_msg);
                    $w_cancel_msg = str_replace('{amount}', moneyFormat($withdrawal->currency->symbol, formatNumber($withdrawal->transaction->total)), $w_cancel_msg);
                    $w_cancel_msg = str_replace('{added/subtracted}', 'added', $w_cancel_msg);
                    $w_cancel_msg = str_replace('{from/to}', 'to', $w_cancel_msg);
                    $w_cancel_msg = str_replace('{soft_name}', Session::get('name'), $w_cancel_msg);
                    ...
                    if (!empty($withdrawal->user->carrierCode) && !empty($withdrawal->user->phone))
                    {
                        ...
                        $t_withdrawal_status_sms_info_suc_ref_msg = str_replace('{status}', ($withdrawal->status == 'Blocked') ? 'Cancelled' : $withdrawal->status, $t_withdrawal_status_sms_info_suc_ref_msg);
                        $t_withdrawal_status_sms_info_suc_ref_msg = str_replace('{amount}', moneyFormat($withdrawal->currency->symbol, formatNumber($withdrawal->transaction->total)), $t_withdrawal_status_sms_info_suc_ref_msg);
                        $t_withdrawal_status_sms_info_suc_ref_msg = str_replace('{added/subtracted}', 'added', $t_withdrawal_status_sms_info_suc_ref_msg);
                        $t_withdrawal_status_sms_info_suc_ref_msg = str_replace('{from/to}', 'to', $t_withdrawal_status_sms_info_suc_ref_msg);
        ...
        /* Bank Transfer - Email */
        $englishBankTransferTempInfoTransferredReceived = EmailTemplate::where(['temp_id' => 7, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first(); //if other language's not set, get en for mail
                ...
                elseif ($t->status == 'Success') //done
                {
                    ...
                    if (!empty($t->bank))
                    {
                        ...
                        if (!empty($t_bank_block_temp->subject) && !empty($t_bank_block_temp->body))
                        {
                            ...
                            $t_block_msg_1 = str_replace('{sender_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_bank_block_temp->body); //
                        }
                        else
                        {
        ...
        $englishTempInfoTransferredReceived = EmailTemplate::where(['temp_id' => 6, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first(); //if other language's not set, get en for mail
        ...
        $t_refund_temp = EmailTemplate::where([
            'temp_id'     => 6,
            'language_id' => Session::get('default_language'),
            'type'        => 'email',
        ])->select('subject', 'body')->first();
        ...
        if ($request->type == 'Transferred')
        {
            ...
            if ($request->status == 'Success')
            {
                ...
                elseif ($t->status == 'Pending') //current status
                {
                    ...
                    if (!empty($t->bank))
                    {
                        ...
                        if (!empty($t_success_temp->subject) && !empty($t_success_temp->body))
                        {
                            $t_success_sub = str_replace('{uuid}', $transfers->uuid, $t_success_temp->subject);
                            $t_success_msg = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_success_temp->body);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (isset($transfers->receiver))
                        {
                            ...
                            {
                                $t_success_sub = str_replace('{uuid}', $transfers->uuid, $t_success_temp->subject);
                                $t_success_msg = str_replace('{sender_id/receiver_id}', $transfers->receiver->first_name . ' ' . $transfers->receiver->last_name, $t_success_temp->body);
                            }
                            else
                            {
                                $t_success_sub = str_replace('{uuid}', $transfers->uuid, $englishTempInfoTransferredReceived->subject);
                ...
                elseif ($t->status == 'Blocked')
                {
                    ...
                    if (!empty($t_success_temp->subject) && !empty($t_success_temp->body))
                    {
                        $t_success_sub_1 = str_replace('{uuid}', $transfers->uuid, $t_success_temp->subject);
                        $t_success_msg_1 = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_success_temp->body);
                    }
                    else
                    {
                    ...
                    if (isset($transfers->receiver))
                    {
                        if (!empty($t_success_temp->subject) && !empty($t_success_temp->body))
                        {
                            $t_success_sub_2 = str_replace('{uuid}', $transfers->uuid, $t_success_temp->subject);
                            $t_success_msg_2 = str_replace('{sender_id/receiver_id}', $transfers->receiver->first_name . ' ' . $transfers->receiver->last_name, $t_success_temp->body);
                        }
                        else
                        {
            ...
            elseif ($request->status == 'Pending')
            {
                ...
                elseif ($t->status == 'Success') //current status
                {
                    ...
                    if (!empty($t->bank))
                    {
                        ...
                        if (!empty($t_pending_temp->subject) && !empty($t_pending_temp->body))
                        {
                            $t_pending_sub = str_replace('{uuid}', $transfers->uuid, $t_pending_temp->subject);
                            $t_pending_msg = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_pending_temp->body);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (isset($transfers->receiver))
                        {
                            ...
                            if (!empty($t_pending_temp->subject) && !empty($t_pending_temp->body))
                            {
                                ...
                                $t_pending_sub = str_replace('{uuid}', $transfers->uuid, $t_pending_temp->subject);
                                // body
                                $t_pending_msg = str_replace('{sender_id/receiver_id}', $transfers->receiver->first_name . ' ' . $transfers->receiver->last_name, $t_pending_temp->body);
                            }
                            else
                            {
                                // subject
            ...
            elseif ($request->status == 'Blocked')
            {
                ...
                elseif ($t->status == 'Success') //current status
                {
                    ...
                    {
                        $t_block_sub_1 = str_replace('{uuid}', $transfers->uuid, $t_block_temp->subject);
                        $t_block_msg_1 = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_block_temp->body); //
                    }
                    else
                    {
                    ...
                    if (isset($transfers->receiver))
                    {
                        $t_block_sub_2 = str_replace('{uuid}', $transfers->uuid, $t_block_temp->subject);
                        $t_block_msg_2 = str_replace('{sender_id/receiver_id}', $transfers->receiver->first_name . ' ' . $transfers->receiver->last_name, $t_block_temp->body);
                ...
                elseif ($t->status == 'Pending')
                {
                    ...
                    // Sent Mail when status is 'blocked'
                    $t_block_sub = str_replace('{uuid}', $transfers->uuid, $t_block_temp->subject);
                    $t_block_msg = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_block_temp->body);
            ...
            elseif ($request->status == 'Refund')
            {
                ...
                elseif ($t->status == 'Success') //current status
                {
                    ...
                    // Mail when refunded
                    if (!empty($t_refund_temp->subject) && !empty($t_refund_temp->body))
                    {
                        $t_refund_sub_1 = str_replace('{uuid}', $transfers->uuid, $t_refund_temp->subject);
                        $t_refund_msg_1 = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_refund_temp->body);
                    }
                    else
                    {
                    ...
                    if (isset($transfers->receiver))
                    {
                        ...
                        {
                            // subject
                            $t_refund_sub_2 = str_replace('{uuid}', $transfers->uuid, $t_refund_temp->subject);
                            // body
                            $t_refund_msg_2 = str_replace('{sender_id/receiver_id}', $transfers->receiver->first_name . ' ' . $transfers->receiver->last_name, $t_refund_temp->body);
                        }
                        else
                        {
                            // subject
        ...
        if ($request->type == 'Received')
        {
            ...
            elseif ($request->status == 'Refund')
            {
                ...
                elseif ($t->status == 'Success') //current status
                {
                    // dd($transferred_row->charge_fixed);
                    ...
                    $t_refund_msg_1 = str_replace('{uuid}', $transfers->uuid, $t_refund_msg_1);
                    $t_refund_msg_1 = str_replace('{status}', ($transfers->status == 'Blocked') ? "Cancelled" : (($transfers->status == 'Refund') ? "Refunded" : $transfers->status), $t_refund_msg_1);
                    $t_refund_msg_1 = str_replace('{amount}', trim(moneyFormat($transferred_row->currency->symbol, formatNumber($transferred_row->total)), '-'), $t_refund_msg_1);
                    $t_refund_msg_1 = str_replace('{added/subtracted}', 'added', $t_refund_msg_1);
                    $t_refund_msg_1 = str_replace('{from/to}', 'to', $t_refund_msg_1);
                    $t_refund_msg_1 = str_replace('{soft_name}', Session::get('name'), $t_refund_msg_1);
        ...
        /**
         * Payment Sent/Payment Received - SMS
         */
        ...
        if ($request->type == 'Payment Sent')
        {
            ...
            if ($request->status == 'Pending')
            {
                ...
                elseif ($t->status == 'Success') //done
                {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $merchant_user_wallet->balance - trim($request->total, '-'),
                    ]);
                    ...
                    if (isset($merchant_payment->merchant))
                    {
                        if (!empty($merchant_status_mail_info->subject) && !empty($merchant_status_mail_info->body))
                        {
                            ...
                            $m_mail_body = str_replace('{paidByUser/merchantUser}', $t->end_user->first_name . ' ' . $t->end_user->last_name, $merchant_status_mail_info->body);
                        }
                        else
                        {
                            $m_mail_sub  = str_replace('{uuid}', $merchant_payment->uuid, $englishTempInfoPaymentSentPaymentReceived->subject);
                            $m_mail_body = str_replace('{paidByUser/merchantUser}', $t->end_user->first_name . ' ' . $t->end_user->last_name, $englishTempInfoPaymentSentPaymentReceived->body);
                        }
                        ...
                        $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, trim($request->total, '-')), $m_mail_body);
                        ...
                        if (checkAppMailEnvironment())
                        {
                            $this->email->sendEmail($t->end_user->email, $m_mail_sub, $m_mail_body);
                        }

                        //sms
                        if (!empty($t->end_user->carrierCode) && !empty($t->end_user->phone))
                        {
                            if (!empty($t_paysentreceived_sms_temp->subject) && !empty($t_paysentreceived_sms_temp->body))
                            {
                                ...
                                $t_paysentreceived_sms_temp_msg = str_replace('{paidByUser/merchantUser}', $t->end_user->first_name . ' ' . $t->end_user->last_name, $t_paysentreceived_sms_temp->body);
                            }
                            else
                            {
                                $t_paysentreceived_sms_temp_sub = str_replace('{uuid}', $merchant_payment->uuid, $t_paysentreceived_en_sms_temp->subject);
                                $t_paysentreceived_sms_temp_msg = str_replace('{paidByUser/merchantUser}', $t->end_user->first_name . ' ' . $t->end_user->last_name, $t_paysentreceived_en_sms_temp->body);
                            }
                            ...
                            $t_paysentreceived_sms_temp_msg = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, trim($request->total, '-')), $t_paysentreceived_sms_temp_msg);
                            ...
                            if (checkAppSmsEnvironment())
                            {
                                if (getNexmoDetails()->is_nexmo_default == 'Yes' && getNexmoDetails()->nexmo_status == 'Active')
                                {
                                    sendSMS(getNexmoDetails()->default_nexmo_phone_number, $t->end_user->carrierCode . $t->end_user->phone, $t_paysentreceived_sms_temp_msg);
                                }
                            }
                        }
                    }
            ...
            elseif ($request->status == 'Success')
            {
                ...
                elseif ($t->status == 'Refund')
                {
                    ...
                    //deduct amount from paid_by_user wallet
                    ...
                    Wallet::where([
                        ...
                        'balance' => $paid_by_user->balance - trim($request->total, '-'),
                    ]);

                    // add amount to merchant_user_wallet wallet
                    $merchant_user_wallet = Wallet::where([
                        'user_id'     => $merchant_payment->merchant->user->id,
                        'currency_id' => $request->currency_id,
                    ])->select('balance')->first();
                    ...
                    {
                        $m_mail_sub  = str_replace('{uuid}', $merchant_payment->uuid, $merchant_status_mail_info->subject);
                        $m_mail_body = str_replace('{paidByUser/merchantUser}', $t->user->first_name . ' ' . $t->user->last_name, $merchant_status_mail_info->body);
                        $m_mail_body = str_replace('{uuid}', $merchant_payment->uuid, $m_mail_body);
                        $m_mail_body = str_replace('{status}', ($merchant_payment->status == 'Blocked') ? "Cancelled" : (($merchant_payment->status == 'Refund') ? "Refunded" : $merchant_payment->status), $m_mail_body);
                        $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, trim($request->total, '-')), $m_mail_body);
                        $m_mail_body = str_replace('{added/subtracted}', 'subtracted', $m_mail_body);
                        ...
                        if (checkAppMailEnvironment())
                        {
                            $this->email->sendEmail($t->user->email, $m_mail_sub, $m_mail_body);
                        }

                        //sms
                        if (!empty($t->user->carrierCode) && !empty($t->user->phone))
                        {
                            if (!empty($t_paysentreceived_sms_temp->subject) && !empty($t_paysentreceived_sms_temp->body))
                            {
                                $t_paysentreceived_sms_temp_sub = str_replace('{uuid}', $merchant_payment->uuid, $t_paysentreceived_sms_temp->subject);
                                $t_paysentreceived_sms_temp_msg = str_replace('{paidByUser/merchantUser}', $t->user->first_name . ' ' . $t->user->last_name, $t_paysentreceived_sms_temp->body);
                            }
                            else
                            {
                                $t_paysentreceived_sms_temp_sub = str_replace('{uuid}', $merchant_payment->uuid, $t_paysentreceived_en_sms_temp->subject);
                                $t_paysentreceived_sms_temp_msg = str_replace('{paidByUser/merchantUser}', $t->user->first_name . ' ' . $t->user->last_name, $t_paysentreceived_en_sms_temp->body);
                            }
                            $t_paysentreceived_sms_temp_msg = str_replace('{status}',($merchant_payment->status == 'Blocked') ? "Cancelled" : (($merchant_payment->status == 'Refund') ? "Refunded" : $merchant_payment->status),$t_paysentreceived_sms_temp_msg);
                            $t_paysentreceived_sms_temp_msg = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, trim($request->total, '-')), $t_paysentreceived_sms_temp_msg);
                            $t_paysentreceived_sms_temp_msg = str_replace('{added/subtracted}', 'subtracted', $t_paysentreceived_sms_temp_msg);
                            ...
                            if (checkAppSmsEnvironment())
                            {
                                if (getNexmoDetails()->is_nexmo_default == 'Yes' && getNexmoDetails()->nexmo_status == 'Active')
                                {
                                    sendSMS(getNexmoDetails()->default_nexmo_phone_number, $t->user->carrierCode . $t->user->phone, $t_paysentreceived_sms_temp_msg);
                                }
                            }
                        }
                    }
            ...
            elseif ($request->status == 'Refund')
            {
                ...
                elseif ($t->status == 'Success') //done
                {
                    ...
                    if (isset($merchant_payment->user_id))
                    {
                        ...
                        {
                            $m_mail_sub  = str_replace('{uuid}', $merchant_payment->uuid, $englishTempInfoPaymentSentPaymentReceived->subject);
                            $m_mail_body = str_replace('{paidByUser/merchantUser}', $t->user->first_name . ' ' . $t->user->last_name, $englishTempInfoPaymentSentPaymentReceived->body);
                        }
                ...
                elseif ($t->status == 'Pending')
                {
                    // dd('current status: Pending, doing Refund');
                    $merchant_payment         = MerchantPayment::find($request->transaction_reference_id);
                    $merchant_payment->status = $request->status;
                    $merchant_payment->save();
        ...
        */
        if ($request->type == 'Payment Received')
        {
            ...
            if ($request->status == 'Pending')
            {
                ...
                elseif ($t->status == 'Success') //done
                {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $merchant_user_wallet->balance - $request->total,
                    ]);
                    ...
                    if (isset($merchant_payment->merchant))
                    {
                        ...
                        $m_mail_body = str_replace('{uuid}', $merchant_payment->uuid, $m_mail_body);
                        $m_mail_body = str_replace('{status}', ($merchant_payment->status == 'Blocked') ? "Cancelled" : (($merchant_payment->status == 'Refund') ? "Refunded" : $merchant_payment->status), $m_mail_body);
                        $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, $request->total), $m_mail_body);
                        $m_mail_body = str_replace('{added/subtracted}', 'subtracted', $m_mail_body);
                        $m_mail_body = str_replace('{from/to}', 'from', $m_mail_body);
                        $m_mail_body = str_replace('{soft_name}', Session::get('name'), $m_mail_body);
            ...
            elseif ($request->status == 'Success')
            {
                ...
                elseif ($t->status == 'Refund')
                {
                    // dd('current status: Refund, doing Success');
                    $merchant_payment         = MerchantPayment::find($request->transaction_reference_id);
                    $merchant_payment->status = $request->status;
                    $merchant_payment->save();
        ...
        if ($request->type == 'Request From')
        {
            ...
            elseif ($request->status == 'Blocked')
            {
                ...
                elseif ($t->status == 'Pending') //current status
                {
                    ...
                    {
                        //Subject
                        $t_block_sub_1 = str_replace('{uuid}', $requestpayment->uuid, $t_block_temp->subject);
                        //Body
                        $t_block_msg_1 = str_replace('{user_id/receiver_id}', $requestpayment->user->first_name . ' ' . $requestpayment->user->last_name, $t_block_temp->body);
                    }
                    else
                    {
                        //Subject
                    ...
                    if (isset($requestpayment->receiver))
                    {
                        ...
                        {
                            //Subject
                            $t_block_sub_2 = str_replace('{uuid}', $requestpayment->uuid, $t_block_temp->subject);
                            //Body
                            $t_block_msg_2 = str_replace('{user_id/receiver_id}', $requestpayment->receiver->first_name . ' ' . $requestpayment->receiver->last_name, $t_block_temp->body);
                        }
                        else
                        {
                            //Subject
            ...
            elseif ($request->status == 'Pending')
            {
                ...
                elseif ($t->status == 'Blocked') //current status
                {
                    ...
                    {
                        //Subject
                        $t_pending_sub_1 = str_replace('{uuid}', $request_payment->uuid, $t_pending_temp_temp->subject);
                        //Body
                        $t_pending_msg_1 = str_replace('{user_id/receiver_id}', $request_payment->user->first_name . ' ' . $request_payment->user->last_name, $t_pending_temp_temp->body);
                    }
                    else
                    {
                        //Subject
                    ...
                    if (isset($request_payment->receiver))
                    {
                        ...
                        {
                            //Subject
                            $t_pending_sub_2 = str_replace('{uuid}', $request_payment->uuid, $t_pending_temp_temp->subject);
                            //Body
                            $t_pending_msg_2 = str_replace('{user_id/receiver_id}', $request_payment->receiver->first_name . ' ' . $request_payment->receiver->last_name, $t_pending_temp_temp->body);
                        }
                        else
                        {
                            //Subject
        ...
        if ($request->type == 'Request To')
        {
            ...
            elseif ($request->status == 'Pending')
            {
                ...
                elseif ($t->status == 'Blocked') //current status
                {
                    ...
                    {
                        //Subject
                        $t_pending_sub_1 = str_replace('{uuid}', $requestpayment->uuid, $t_pending_temp_temp->subject);
                        //Body
                        $t_pending_msg_1 = str_replace('{user_id/receiver_id}', $requestpayment->user->first_name . ' ' . $requestpayment->user->last_name, $t_pending_temp_temp->body);
                    }
                    else
                    {
                        //Subject
                    ...
                    if (isset($requestpayment->receiver))
                    {
                        ...
                        {
                            //Subject
                            $t_pending_sub_2 = str_replace('{uuid}', $requestpayment->uuid, $t_pending_temp_temp->subject);
                            //Body
                            $t_pending_msg_2 = str_replace('{user_id/receiver_id}', $requestpayment->receiver->first_name . ' ' . $requestpayment->receiver->last_name, $t_pending_temp_temp->body);
                        }
                        else
                        {
            ...
            if ($request->status == 'Success')
            {
                ...
                elseif ($t->status == 'Blocked')
                {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $receiver_wallet->balance + trim($request->total, '-') * $exchange->toWallet->currency->rate,
                    ]);
                    $this->helper->one_time_message('success', 'Transaction Updated Successfully!');
                }
            }
            ...
            elseif ($request->status == 'Success')
            {
                ...
                elseif ($t->status == 'Pending') //current status
                {
                    ...
                    if (isset($t_success_temp))
                    {
                        ...
                        // body
                        $t_success_msg = str_replace('{activator_id}', $voucher->activator->first_name . ' ' . $voucher->activator->last_name, $t_success_temp->body);
