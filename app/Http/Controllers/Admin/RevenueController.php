...
class RevenueController extends Controller
{
    ...
    public function revenues_list(RevenuesDataTable $dataTable)
    {
        ...
        $data['revenues_status']   = $this->revenue->select('status')->where('charge_percentage', '!=', 0)->where('charge_fixed', '!=', 0)->whereIn('transaction_type_id', [Deposit, Withdrawal, Transferred, Request_To])->groupBy('status')->get();
        $data['revenues_currency'] = $this->revenue->select('currency_id')->where('charge_percentage', '!=', 0)->where('charge_fixed', '!=', 0)->whereIn('transaction_type_id', [Deposit, Withdrawal, Transferred, Request_To])->groupBy('currency_id')->get();
        $data['revenues_type']     = $this->revenue->select('transaction_type_id')->where('charge_percentage', '!=', 0)->where('charge_fixed', '!=', 0)->whereIn('transaction_type_id', [Deposit, Withdrawal, Transferred, Request_To])->groupBy('transaction_type_id')->get();
        ...
        $getRevenuesListForCurrencyIfo = $this->revenue->getRevenuesListForCurrencyInfoAndCsvPdf($from, $to, $status, $currency, $type);
    ...
    public function revenueCsv()
    {
        ...
        $data['revenues'] = $revenues = $this->revenue->getRevenuesListForCurrencyInfoAndCsvPdf($from, $to, $status, $currency, $type);
        // $data['revenues'] = $revenues = $this->revenue->getRevenuesList($from, $to, $status, $currency, $type)->orderBy('transactions.id', 'desc')->get(); //using get and orderby in object -optimized
...
// ->with(array('payment_method' => function($p) use ($pm) {
//     $p->where('id', $pm);
// },
