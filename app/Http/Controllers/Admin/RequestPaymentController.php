    ...
    public function requestpaymentCsv()
    {
        ...
        if (!empty($requestpayments))
        {
            foreach ($requestpayments as $key => $value)
            {
                ...
                $datas[$key]['Receiver'] = isset($value->receiver) ? $value->receiver->first_name . ' ' . $value->receiver->last_name : "-";
        ...
        if ($request->transaction_type == 'Request_To')
        {
            ...
            elseif ($request->status == 'Blocked') //when current status is 'cancelled'
            {
                ...
                if (isset($userInfo))
                {
                    ...
                    $t_block_msg_2 = str_replace('{status}', ($requestpayment->status == 'Blocked') ? "Cancelled" : (($requestpayment->status == 'Refund') ? "Refunded" : $requestpayment->status), $t_block_msg_2);
            ...
            elseif ($request->status == 'Pending') //when current status is 'pending'
            {
                ...
                $t_pending_msg_1 = str_replace('{status}', ($requestpayment->status == 'Blocked') ? "Cancelled" : (($requestpayment->status == 'Refund') ? "Refunded" : $requestpayment->status), $t_pending_msg_1);
