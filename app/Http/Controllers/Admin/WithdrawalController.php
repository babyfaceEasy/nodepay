...
class WithdrawalController extends Controller
{
    ...
    public function withdrawalCsv()
    {
        ...
        if (!empty($withdrawals))
        {
            foreach ($withdrawals as $key => $value)
            {
                ...
                // $datas[$key]['Method Info'] = $value->payment_method_info;
    ...
    public function update(Request $request)
    {
        ...
        $englishSenderLanginfo = EmailTemplate::where(['temp_id' => 10, 'lang' => 'en', 'type' => 'email'])->select('subject', 'body')->first();

        /*
        Will modify these 3 queries to 1 later
         */
        $withdrawal_pending_temp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type' => 'email',
        ])->select('subject', 'body')->first();

        $withdrawal_success_temp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type' => 'email',
        ])->select('subject', 'body')->first();

        $withdrawal_cancel_temp = EmailTemplate::where([
            'temp_id'     => 10,
            'language_id' => Session::get('default_language'),
            'type' => 'email',
        ])->select('subject', 'body')->first();
        ...
        if ($request->transaction_type == 'Withdrawal')
        {
            if ($request->status == 'Pending') //requested status
            {
                ...
                elseif ($request->transaction_status == 'Success')
                {
                    ...
                    if (!empty($withdrawal_pending_temp->subject) && !empty($withdrawal_pending_temp->body))
                    {
                        // subject
                        $withdrawal_pending_sub = str_replace('{uuid}', $withdrawal->uuid, $withdrawal_pending_temp->subject);
                        // body
                        $withdrawal_pending_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $withdrawal_pending_temp->body);
                    }
                    else
                    {
                        // subject
                        $withdrawal_pending_sub = str_replace('{uuid}', $withdrawal->uuid, $englishSenderLanginfo->subject);
                        // body
                        $withdrawal_pending_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishSenderLanginfo->body);
                    }
            ...
            elseif ($request->status == 'Success')
            {
                ...
                elseif ($request->transaction_status == 'Pending')
                {
                    ...
                    //Mail when withdrawal is 'Success'
                    if (!empty($withdrawal_success_temp->subject) && !empty($withdrawal_success_temp->body))
                    {
                        // subject
                        $withdrawal_success_sub = str_replace('{uuid}', $withdrawal->uuid, $withdrawal_success_temp->subject);
                        // body
                        $withdrawal_success_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $withdrawal_success_temp->body);
                    }
                    else
                    {
                        // subject
                        $withdrawal_success_sub = str_replace('{uuid}', $withdrawal->uuid, $englishSenderLanginfo->subject);
                        // body
                        $withdrawal_success_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishSenderLanginfo->body);
                    }
            ...
            elseif ($request->status == 'Blocked')
            {
                ...
                elseif ($request->transaction_status == 'Pending') //current status
                {
                    ...
                    //Mail when withdrawal is 'Success'
                    if (!empty($withdrawal_cancel_temp->subject) && !empty($withdrawal_cancel_temp->body))
                    {
                        // subject
                        $withdrawal_cancel_sub = str_replace('{uuid}', $withdrawal->uuid, $withdrawal_cancel_temp->subject);
                        // body
                        $withdrawal_cancel_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $withdrawal_cancel_temp->body);
                    }
                    else
                    {
                        // subject
                        $withdrawal_cancel_sub = str_replace('{uuid}', $withdrawal->uuid, $englishSenderLanginfo->subject);
                        // body
                        $withdrawal_cancel_msg = str_replace('{user_id}', $withdrawal->user->first_name . ' ' . $withdrawal->user->last_name, $englishSenderLanginfo->body);
                    }
