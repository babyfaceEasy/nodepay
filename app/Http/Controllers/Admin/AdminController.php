...
class AdminController extends Controller
{
    ...
    {
        $data['menu']   = 'profile';
        $data['admins'] = $admins = Admin::where(['id' => Auth::guard('admin')->user()->id])->first();
    ...
    public function updatePassword(Request $request, $id)
    {
        ...
        if (\Hash::check($request->old_pass, $admins->password))
        {
            Admin::where(['id' => $id])->update($data);
        ...
        else
        {
            ...
            return redirect()->intended("admin/change-password/$id");
        }
    }
