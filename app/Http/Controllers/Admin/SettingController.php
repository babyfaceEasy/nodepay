...
class SettingController extends Controller
{
    ...
    public function general(Request $request)
    {
        ...
        else if ($_POST)
        {
            ...
            else
            {
                ...
                Setting::where(['name' => 'head_code'])->update(['value' => is_null($request->head_code) ? '' : $request->head_code]);
                ...
                $feeslimitArray = FeesLimit::where(['currency_id' => $request->default_currency])->pluck('transaction_type_id')->toArray();
                // dd($feeslimitArray);

                $arr = [];
                foreach ($transaction_types as $key => $value)
                {
                    if (!in_array($value, $feeslimitArray))
                    {
                        $arr[] = $value;
                    }
                }
                ...
                if (!empty($arr))
                {
                    for ($i = 0; $i < count($arr); $i++)
                    {
                        ...
                        $feesLimit->has_transaction     = 'Yes';
                        $feesLimit->transaction_type_id = $arr[$i];
                        $feesLimit->save();
                    }
                }
                /*
    ...
    public function checkSmsGatewaySettings(Request $request)
    {
        ...
        // $checkUser = User::get(['phone']);
        // if ($checkUser->isEmpty())
        // {
        //     return response()->json([
        //         'status'  => 404,
        //         'message' => 'Warning! One or more user do not have phone number!',
        //     ]);
        // }
        // else
        // {
        //     return response()->json([
        //         'status'  => 200,
        //         'message' => 'Warning! Phone Numbers Exist!',
        //     ]);
        // }
