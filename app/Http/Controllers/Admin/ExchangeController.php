...
class ExchangeController extends Controller
{
    ...
    public function index(CurrencyExchangesDataTable $dataTable)
    {
        ...
        return $dataTable->with('defaultCurrencySymbol', $defaultCurrency->symbol)->render('admin.exchange.list', $data);
    }
    ...
    {
        $def_curr_code = isset($_GET['def_curr_code']) ? $_GET['def_curr_code'] : null;
        // dd($def_curr_code);

        $def_curr_symbol = isset($_GET['def_curr_symbol']) ? $_GET['def_curr_symbol'] : null;
        // dd($def_curr_symbol);
        ...
        if (!empty($exchanges))
        {
            foreach ($exchanges as $key => $value)
            {
                ...
                if ($value->type == 'Out')
                {
                    if ($value->amount > 0)
                    {
                        $datas[$key]['Amount'] = moneyFormat($def_curr_symbol, formatNumber($value->amount));
                    }
                }
                elseif ($value->type == 'In')
                {
                    if ($value->amount > 0)
                    {
                        $datas[$key]['Amount'] = moneyFormat($value->tc_symbol, formatNumber($value->amount));
                    }
                }
                ...
                if($value->type == 'Out')
                {
                    ...
                    {
                      $datas[$key]['Total'] = moneyFormat($def_curr_symbol, formatNumber($value->fee + $value->amount));
                    }
                    else
                    {
                      $datas[$key]['Total'] = '-';
                ...
                elseif($value->type == 'In')
                {
                    if (($value->fee + $value->amount) > 0)
                    {
                      $datas[$key]['Total'] = moneyFormat($value->tc_symbol, formatNumber($value->fee + $value->amount));
                    }
                    else
                    {
                      $datas[$key]['Total'] = '-';
    ...
    public function exchangePdf()
    {
        ...
        $data['def_curr_code'] = $def_curr_code = isset($_GET['def_curr_code']) ? $_GET['def_curr_code'] : null;

        $data['def_curr_symbol'] = $def_curr_symbol = isset($_GET['def_curr_symbol']) ? $_GET['def_curr_symbol'] : null;
    ...
    {
        // dd($request->user_id);
        // dd($exchange->fromWallet->currency->id);
        // dd($request->transaction_reference_id);
            ...
            if ($request->status == 'Success')
            {
                ...
                elseif ($request->transaction_status == 'Blocked')
                {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $to_wallet->balance + ($request->amount * $exchange->toWallet->currency->rate),
                    ]);
