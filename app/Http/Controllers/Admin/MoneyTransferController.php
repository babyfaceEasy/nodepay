    ...
    public function update(Request $request)
    {
        ...
        if ($request->transaction_type == 'Transferred')
        {
                ...
                elseif ($request->transaction_status == 'Success') //current status
                {
                    ...
                    if (isset($userInfo))
                    {
                        ...
                        if (!empty($t_pending_temp->subject) && !empty($t_pending_temp->body))
                        {
                            ...
                            $t_pending_msg = str_replace('{sender_id/receiver_id}', $userInfo->first_name . ' ' . $userInfo->last_name, $t_pending_temp->body);
                        }
                        else
                        {
                ...
                elseif ($request->transaction_status == 'Pending') //current status
                {
                    ...
                    if (isset($userInfo))
                    {
                        ...
                        if (!empty($t_success_temp->subject) && !empty($t_success_temp->body))
                        {
                            ...
                            // body
                            $t_success_msg = str_replace('{sender_id/receiver_id}', $userInfo->first_name . ' ' . $userInfo->last_name, $t_success_temp->body);
                        }
                        else
                        {
                            // subject
                ...
                elseif ($request->transaction_status == 'Blocked')
                {
                    ...
                    if (isset($userInfo))
                    {
                        ...
                        if (!empty($t_success_temp->subject) && !empty($t_success_temp->body))
                        {
                            ...
                            // body
                            $t_success_msg_2 = str_replace('{sender_id/receiver_id}', $userInfo->first_name . ' ' . $userInfo->last_name, $t_success_temp->body);
                        }
                        else
                        {
                            // subject
            ...
            elseif ($request->status == 'Blocked')
            {
                ...
                elseif ($request->transaction_status == 'Success')
                {
                    ...
                    if (!empty($t_block_temp->subject) && !empty($t_block_temp->body))
                    {
                        ...
                        $t_block_msg_1 = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_block_temp->body); // body
                    }
                    else
                    {
                        // subject
                    ...
                    if (isset($userInfo))
                    {
                        ...
                        if (!empty($t_block_temp->subject) && !empty($t_block_temp->body))
                        {
                            ...
                            // body
                            $t_block_msg_2 = str_replace('{sender_id/receiver_id}', $userInfo->first_name . ' ' . $userInfo->last_name, $t_block_temp->body);
                        }
                        else
                        {
                            // subject
            ...
            elseif ($request->status == 'Refund')
            {
                ...
                elseif ($request->transaction_status == 'Success') //done
                {
                    ...
                    if (isset($userInfo))
                    {
                        if (!empty($t_refund_temp->subject) && !empty($t_refund_temp->body))
                        {
                            ...
                            // body
                            $t_refund_msg_2 = str_replace('{sender_id/receiver_id}', $userInfo->first_name . ' ' . $userInfo->last_name, $t_refund_temp->body);
                        }
                        else
                        {
                            // subject
                ...
                elseif ($request->transaction_status == 'Pending')
                {
                    // dd('Transferred => Request: Refund, DB Status: Pending');
                    $transfers         = Transfer::find($request->id);
                    $transfers->status = $request->status;
                    $transfers->save();
                    ...
                    {
                        // subject
                        $t_refund_sub = str_replace('{uuid}', $transfers->uuid, $t_refund_temp->subject);
                        // body
                        $t_refund_msg = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $t_refund_temp->body);
                    }
                    else
                    {
                        // subject
                        $t_refund_sub = str_replace('{uuid}', $transfers->uuid, $englishSenderLanginfo->subject);
                        // body
                        $t_refund_msg = str_replace('{sender_id/receiver_id}', $transfers->sender->first_name . ' ' . $transfers->sender->last_name, $englishSenderLanginfo->body);
                    }
                    $t_refund_msg = str_replace('{uuid}', $transfers->uuid, $t_refund_msg);
                    $t_refund_msg = str_replace('{status}', ($transfers->status == 'Blocked') ? "Cancelled" : (($transfers->status == 'Refund') ? "Refunded" : $transfers->status), $t_refund_msg);
                    $t_refund_msg = str_replace('{amount}', moneyFormat($transfers->currency->symbol, formatNumber($request->amount + $request->feesTotal)), $t_refund_msg);
                    $t_refund_msg = str_replace('{added/subtracted}', 'added', $t_refund_msg);
                    $t_refund_msg = str_replace('{from/to}', 'to', $t_refund_msg);
                    $t_refund_msg = str_replace('{soft_name}', Session::get('name'), $t_refund_msg);

                    if (checkAppMailEnvironment())
                    {
                        $this->email->sendEmail($transfers->sender->email, $t_refund_sub, $t_refund_msg);
                    }
