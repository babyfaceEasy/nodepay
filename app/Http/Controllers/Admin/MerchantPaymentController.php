...
class MerchantPaymentController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        // dd($merchant_payment->merchant);

        $data['transactionOfRefunded'] = $transactionOfRefunded = Transaction::select('refund_reference')->where(['uuid' => $merchant_payment->uuid])->first();
    ...
    public function update(Request $request)
    {
        ...
        if ($request->transaction_type == 'Payment_Sent')
        {
            ...
            if ($request->status == 'Pending')
            {
                ...
                elseif ($request->transaction_status == 'Success') //current status
                {
                    ...
                    Wallet::where([
                        ...
                        'balance' => $merchant_user_wallet->balance - ($request->amount + $request->charge_percentage + $request->charge_fixed),
                    ]);
                    ...
                    if (isset($merchant_payment->merchant))
                    {
                        ...
                        $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, ($request->amount + $request->charge_percentage + $request->charge_fixed)), $m_mail_body);
                ...
                //         $m_mail_body = str_replace('{uuid}', $merchant_payment->uuid, $m_mail_body);
                //         $m_mail_body = str_replace('{status}', $merchant_payment->status, $m_mail_body);
                //         $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, $request->amount), $m_mail_body);
                //         $m_mail_body = str_replace('{added/subtracted}', 'subtracted', $m_mail_body);
                //         $m_mail_body = str_replace('{from/to}', 'from', $m_mail_body);
                //         $m_mail_body = str_replace('{soft_name}', Session::get('name'), $m_mail_body);
            ...
            elseif ($request->status == 'Success')
            {
                ...
                //         'currency_id' => $request->currency_id,
                //     ])->update([
                //         'balance' => $paid_by_user->balance - $request->amount,
                //     ]);
            ...
            elseif ($request->status == 'Refund')
            {
                ...
                elseif ($request->transaction_status == 'Success') //done
                {
                    ...
                    $merchant_payment->total             = $request->amount + ($request->charge_percentage + $request->charge_fixed);
                    ...
                    Wallet::where([
                        ...
                        // 'balance' => $merchant_user_wallet->balance - $request->amount,
                        'balance' => $merchant_user_wallet->balance - ($request->charge_percentage + $request->charge_fixed + $request->amount),
                    ]);
                    ...
                    if (isset($merchant_payment->user_id))
                    {
                        ...
                        $m_mail_body = str_replace('{uuid}', $merchant_payment->uuid, $m_mail_body);
                        $m_mail_body = str_replace('{status}', $merchant_payment->status, $m_mail_body);
                        $m_mail_body = str_replace('{amount}', moneyFormat($merchant_payment->currency->symbol, ($request->charge_percentage + $request->charge_fixed + $request->amount)), $m_mail_body);
                        $m_mail_body = str_replace('{added/subtracted}', 'added', $m_mail_body);
                        $m_mail_body = str_replace('{from/to}', 'to', $m_mail_body);
                        $m_mail_body = str_replace('{soft_name}', Session::get('name'), $m_mail_body);
