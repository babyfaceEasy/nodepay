...
class DepositController extends Controller
{
    ...
    public function index(DepositsDataTable $dataTable)
    {
        ...
        $data['d_currencies'] = $d_currencies = $this->deposit->select('currency_id')->groupBy('currency_id')->get();
        $data['d_pm']         = $d_pm         = $this->deposit->select('payment_method_id')->whereNotNull('payment_method_id')->groupBy('payment_method_id')->get();
    ...
    public function depositCsv()
    {
        ...
        if (!empty($deposits))
        {
            foreach ($deposits as $key => $value)
            {
                ...
                $datas[$key]['Fees'] = formatNumber($value->charge_percentage + $value->charge_fixed);
