...
class UserTransactionController extends Controller
{
    ...
    public function getTransaction(Request $request)
    {
        ...
        if ($transaction->count() > 0)
        {
            ...
            {
                $cancel_btn = '';
                // if ($transaction->status != 'Blocked' && $transaction->status != 'Success')
                if ($transaction->status == 'Pending')
                {
                    $cancel_btn = "<button class='btn btn-secondary btn-sm trxnreqfrom' data='" . $transaction->id . "' data-type='" . $transaction->transaction_type_id . "' id='btn_" . $transaction->id . "'>" . __('message.form.cancel') . "</button>";

                    // $cancel_btn = "<button class='btn btn-secondary btn-sm trxn' data='" . $transaction->id . "' data-type='" . $transaction->transaction_type_id . "' id='btn_" . $transaction->id . "'>".__('message.form.cancel')."</button>";
                }
            ...
            else if ($transaction->transaction_type_id == Request_To)
            {
                ...
                {
                    $cancel_btn = "<button class='btn btn-secondary btn-sm trxn' data='" . $transaction->id . "' data-type='" . $transaction->transaction_type_id . "' id='btn_" . $transaction->id . "'>" . __('message.form.cancel') . "</button>";
                    $cancel_btn .= " <button class='btn btn-secondary btn-sm trxn_accept' data-rel='" . $transaction->transaction_reference_id . "' data='" . $transaction->id . "' id='acceptbtn_" . $transaction->id . "'> " . __('message.dashboard.left-table.request-to.accept') . " </button>";
                }
                if ($transaction->user_type == 'registered')
                {
                    $data['html'] = "<div class='form-group trans_details'>" .
