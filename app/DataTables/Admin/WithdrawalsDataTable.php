...
class WithdrawalsDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return datatables()
            ...
            {
                return formatNumber($withdrawal->charge_percentage + $withdrawal->charge_fixed);
            })
            ->addColumn('total', function ($withdrawal)
            {
                if (($withdrawal->charge_percentage + $withdrawal->charge_fixed + $withdrawal->amount) > 0)
                {
                    $total = '<td><span class="text-green">+' . formatNumber($withdrawal->charge_percentage + $withdrawal->charge_fixed + $withdrawal->amount) . '</span></td>';
                }
                else
                {
