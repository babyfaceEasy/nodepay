...
class CurrenciesDataTable extends DataTable
{
    public function ajax() //don't use default dataTable() method
    {
        return datatables()
            ...
            ->editColumn('logo', function ($currency)
            {
                if ($currency->logo)
                {
                    $logo = '<td><img src="'. url('public/uploads/currency_logos/' . $currency->logo).'" width="70" height="50" class="img-responsive"></td>';
                }
                else
                {
                    $logo = '<td><img src="'. url('public/user_dashboard/images/favicon.png').'" width="70" height="50" class="img-responsive"></td>';
                }
                return $logo;
            })
            ...
            ->addColumn('action', function ($currency)
            {
                ...
                $feesLimit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url('admin/settings/feeslimit/deposit/' . $currency->id) . '" class="btn btn-xs btn-success">
                <i class="glyphicon glyphicon-view">Fees</i></a>&nbsp;' : '';

                $pm = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url('admin/settings/payment-methods/stripe/' . $currency->id) . '" class="btn btn-xs btn-primary">
    ...
    {
        // $query = Currency::select(['id', 'name', 'code', 'symbol', 'rate', 'logo', 'status']);
