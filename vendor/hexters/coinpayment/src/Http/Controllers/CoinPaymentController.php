        ...
        if (auth()->check())
        {
            ...
            if ($payment['error'] == 'ok' && (INT) $user->coinpayment_transactions()->where('payment_id', $req->result['txn_id'])->count('id') === 0)
            {
                ...
                if (isset($req->payload['type']) && $req->payload['type'] == "merchant")
                {
                    ...
                    $merchantPayment->amount            = Session::get('amount');
                ...
                elseif (isset($req->payload['type']) && $req->payload['type'] == "deposit")
                {

                    $payment_method_id = Session::get('payment_method_id');
                    $amount            = Session::get('amount');
                    $curr              = Currency::where('code', Session::get('coinpayment_currency'))->first();
                    $currencyId        = $curr->id;
                    ...
                    $deposits->uuid              = $uuid;
                    $deposits->charge_percentage = @$feeInfo->charge_percentage ? (($amount) * (@$feeInfo->charge_percentage / 100)) : 0;
                    ...
                    $payload                   = empty($req->payload) ? [] : $req->payload;
                    $payload['deposit_id']     = $deposits->id;
