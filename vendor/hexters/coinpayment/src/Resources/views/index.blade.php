                            ...
                            <div class="card-body">
                                <div class="text-center">
                                    @if(config('coinpayment.header_type') == 'logo')
                                        <img  class="img-responsive img-fluid" src="{{url('public/images/logos/'.getCompanyLogo())}}"
style="background: rgba(74, 111, 197, 0.9) !important;">
                                            ...
                                            <td>
                                                ...
                                                <div>
                                                    <small class="text-muted">Item
                                                        Price: {{ $item['priceItem'] }} {{ config('coinpayment.default_currency') }}</small>
                                                    <br>
                                ...
                                <table class="table">
                                    ...
                                    <tr>
                                        <td class="text-right">Total
                                            Amount {{ config('coinpayment.default_currency') }}</td>
                        ...
                        <div class="text-center">
                            <button type="button" v-bind:disabled="!(coins.length > 0)"
                                    class="btn btn-block btn-danger mt-3 mb-4" @click="confirmation" style=
                            "background: red
                            ;" name="button">Pay Now <i class="fa fa-arrow-circle-right"></i></button>
                            <div class="text-muted">
                                <a href="{{ url('/dashboard') }}" class="text-muted">Cancel your payment</a> | <a class="text-muted" href="{{ route('coinpayment.transaction.histories') }}">Transaction Histories</a>
                            </div>
                        </div>
                    </div>
                    <div class="col-sm-8">
                        <div class="card">
                            <div class="card-body">
                                <div class="input-group mb-3 form-search">
                                    <input v-model="searchCoin" autofocus type="search" placeholder="Search Coin..."
                                           value="" class="form-control form-search-input">
                    ...
                    <span class="input-group-text" id="basic-addon1">
                      <i class="fa fa-search"></i>
                                ...
                                <div class="row coin-items">
                                    <div class="col-sm-6 show-coin mt-3" v-for="(coin, index) in filterCoin()"
                                         :key="coin.name" @click="selectMethod(coin)">
                                    <div class="card" v-bind:class="{ 'selected' : coin.selected }">
                                        <div class="media mt-2 ml-2 mr-2 mb-3">
                                            <img class="mr-3" width="40" v-bind:src="changeIcon(coin.icon)">
    ...
    <div class="modal" id="paynow" data-backdrop="static" tabindex="-1" role="dialog"
                ...
                <div class="table-responsive col-sm-">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td>@{{ payment.first.amount }} @{{ payment.last.coin }}
                                <span>(total confirms needed: @{{ payment.first.confirms_needed }})</span></td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <td class="text-center text-muted" colspan="2">
                                <a class="text-muted" v-bind:href="payment.first.status_url" target="_blank">Alternative
                                    Link</a> | <a class="text-muted"
                                                  href="{{ route('coinpayment.transaction.histories') }}">Transaction
                                    Histories</a>
                            </td>
                        </tr>
                    </table>
...
<script type="text/javascript">
    ...
    var vue = new Vue({
        ...
        data: {
            ...
            currency:'{{session('coinpayment_currency')}}',
            coins: [],
            errors: [],
            isError: false,
            coinAliases: [],
        ...
        methods: {
            getMethods(){
                var self = this;
                axios.get('{{ route('coinpayment.ajax.rate.usd',['amount'=>$data['amountTotal'],'currency'=>session('coinpayment_currency')]) }}')
                    .then(function (json) {
